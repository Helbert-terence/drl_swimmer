...
class SwimmerEnv:
    ...
    def get_state(self):
        ...
        # Relative angles between consecutive segments
        ...
        # Normalized direction to the prey
        state.append(dx/dist)
        state.append(dy/dist)
        # Normalized distance
        state.append(dist/800)
        # Cos(angle between the head heading and the prey direction)
    ...
    def step(self, torques):
        ...
        # Head velocity projected on the direction to the prey
        head = self.bodies[0]
        ...
        reward = vel_toward/50.0    # positive when it approaches, negative when it moves away
        reward -= 0.02              # time penalty: an immobile creature has a negative reward
        if dist < 15:
            reward += 5.0
            self._respawn_prey()
    ...
    def render(self):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                self.screen = None      # stop the rendering
                pygame.display.quit()   # close the window only
                return

        self.screen.fill((20, 60, 120))     # water blue background
